...
class Login extends CI_Controller
{
    ...
    function registerMe(){
        ...
        $this->form_validation->set_rules('email', 'Email', 'required|valid_email|max_length[128]|trim');
        $this->form_validation->set_rules('password', 'Password', 'required|max_length[32]');
        $this->form_validation->set_rules('pancard', 'Pancard', 'required|max_length[16]');
        
        if($this->form_validation->run() == FALSE)
        {
            $this->partner_register();
        }else{
            $dataArray=array(
			 ...
			 if($lastId)
			 {
                $this->session->set_flashdata('success', 'Registration has been successfully');
                redirect('/login');
			 }else{
